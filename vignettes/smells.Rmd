...
The `xlex()` function separates an Excel formula into words, or 'tokens'.  Its
name is a bad pun on 'Excel' and 'lexer'.

This is useful for detecting spreadsheet smells, which are poor practices in
...
If you have the `igraph` and `ggraph` packages installed
(`install.packages(c("igraph", "ggraph"))`), then you can visualise the parse
tree with `plot_xlex()`.
...
Intermediate functions between `xlex()` and `plot_xlex()` that work on the parse
tree are also exported: `xlex_edges()`, `xlex_vertices()` and `xlex_igraph()`.
...
Hermans](http://www.felienne.com/archives/3634).
