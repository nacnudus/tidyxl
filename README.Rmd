...
[tidyxl](https://github.com/nacnudus/tidyxl) imports non-tabular data from Excel files into R.  It exposes cell content,
position, formatting and comments in a tidy structure for further manipulation,
especially by the [unpivotr](https://github.com/nacnudus/unpivotr) package.  It
...
Specific sheets can be requested using `tidy_xlsx(file, sheet)`, and the names
...
instead the `formula_type` column has the value `array`.
...
names to the cells returned by `tidy_xlsx()` -- e.g. by using the
...
control what values may be entered into a cell, and are often used to create
a drop-down list in a cell.  Because they are defined for cells on particular
sheets, the returned data structure is similar to `tidy_xlsx()`, i.e. a
list of data frames, named by the worksheet.
